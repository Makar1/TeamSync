...
from app.services.meeting import has_overlap, update_meeting_with_participants, InvalidTimeRangeError, MeetingConflictError
...
def update_meeting(meeting_id: int, meeting_data: MeetingUpdate, db: Session = Depends(get_db), membership: TeamMember = Depends(get_team_membership)):
    ...
    if meeting is None:
        raise HTTPException(status_code=404, detail="Нет такой встречи")
    if not (membership.role == TeamRole.MANAGER or meeting.organizer_id == membership.user_id):
        raise HTTPException(status_code=403, detail="Только менеджер или организатор может менять встречу")

    participant_ids = set(meeting_data.participant_ids) if meeting_data.participant_ids is not None else None
    try:
        new_participant_ids = update_meeting_with_participants(
            db, meeting, meeting_data.title, meeting_data.starts_at, meeting_data.ends_at, participant_ids
        )
    ...
    return MeetingResponse(
        id=meeting.id, title=meeting.title, team_id=meeting.team_id,
        starts_at=meeting.starts_at, ends_at=meeting.ends_at,
        organizer_id=meeting.organizer_id, created_at=meeting.created_at,
